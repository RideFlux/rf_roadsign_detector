...
def bbox3d2corners(bboxes):
    ...
    '''
    centers, dims, angles = bboxes[:3], bboxes[3:6], 0

    # 1.generate bbox corner coordinates, clockwise from minimal point
    bboxes_corners = np.array([[-0.5, -0.5, 0], [-0.5, -0.5, 1.0], [-0.5, 0.5, 1.0], [-0.5, 0.5, 0.0],
                               [0.5, -0.5, 0], [0.5, -0.5, 1.0], [0.5, 0.5, 1.0], [0.5, 0.5, 0.0]], 
                               dtype=np.float32)
    bboxes_corners = bboxes_corners[:, :] * dims[None, :] # (1, 8, 3) * (n, 1, 3) -> (n, 8, 3)

    bboxes_corners = bboxes_corners # (n, 8, 3)
...
def nearest_bev(bboxes):
    '''
    bboxes: (n, 6), (x, y, z, w, l, h, theta)
    return: (n, 4), (x1, y1, x2, y2)
    '''    
    bboxes_bev = copy.deepcopy(bboxes[:, [0, 1, 3, 4]])
    bboxes_bev = torch.where(torch.abs(torch.Tensor([0]).to(bboxes_bev)) > np.pi / 4, bboxes_bev[:, [0, 1, 3, 2]], bboxes_bev)
...
def iou2d(bboxes1, bboxes2, metric=0, return_distance_if_no_overlap=False):
    '''
    ...
    if return_distance_if_no_overlap:
        bboxes1_center = (bboxes1[:, :2] + bboxes1[:, 2:]) / 2  # (n, 2)
        bboxes2_center = (bboxes2[:, :2] + bboxes2[:, 2:]) / 2  # (m, 2)
        
        center_distances = torch.sqrt(
            (bboxes1_center[:, 0:1] - bboxes2_center[None, :, 0]) ** 2 +
            (bboxes1_center[:, 1:2] - bboxes2_center[None, :, 1]) ** 2
        )  # (n, m)
        
        zero_iou_mask = (iou == 0)
        result = torch.where(zero_iou_mask, -center_distances, iou)
        return result
...
def iou2d_nearest(bboxes1, bboxes2):
    ...
    iou = iou2d(bboxes1_bev, bboxes2_bev, return_distance_if_no_overlap=True)
    return iou

def dist2d(bboxes1, bboxes2):
    bboxes1 = nearest_bev(bboxes1)
    bboxes2 = nearest_bev(bboxes2)

    bboxes1_center = (bboxes1[:, :2] + bboxes1[:, 2:]) / 2  # (n, 2)
    bboxes2_center = (bboxes2[:, :2] + bboxes2[:, 2:]) / 2  # (m, 2)
    
    return bboxes1_center[:, 0:1] - bboxes2_center[None, :, 0],\
           bboxes1_center[:, 1:2] - bboxes2_center[None, :, 1]
