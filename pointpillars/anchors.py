...
class Anchors():
    ...
    def get_anchors(self, feature_map_size, anchor_range, anchor_size):
        '''
        feature_map_size: (y_l, x_l)
        anchor_range: [x1, y1, z1, x2, y2, z2]
        anchor_size: [w, l, h]
        return: shape=(y_l, x_l, 6) - [x, y, z, w, l, h]
        '''
        ...
        # [feature_map_size[1], feature_map_size[0], 1] * 3
        meshgrids = torch.meshgrid(x_centers, y_centers, z_centers, indexing='ij')
        meshgrids = list(meshgrids)
        for i in range(len(meshgrids)):
            meshgrids[i] = meshgrids[i][..., None] # [feature_map_size[1], feature_map_size[0], 1, 1]
        
        anchor_size = anchor_size[None, None, None, :]
        repeat_shape = [feature_map_size[1], feature_map_size[0], 1, 1]
        anchor_size = anchor_size.repeat(repeat_shape) # [feature_map_size[1], feature_map_size[0], 1, 3]
        meshgrids.append(anchor_size)
        anchors = torch.cat(meshgrids, dim=-1).permute(2, 1, 0, 3).contiguous() # [1, feature_map_size[0], feature_map_size[1], 6]
    ...
    def get_multi_anchors(self, feature_map_size):
        ...
        return: shape=(y_l, x_l, 3, 6) - [x, y, z, w, l, h]
        '''
        device = feature_map_size.device
...
def anchors2bboxes(anchors, deltas):
    ...
    bboxes = torch.stack([x, y, z, w, l, h], dim=1)
    return bboxes
...
def bboxes2deltas(bboxes, anchors):
    ...
    deltas = torch.stack([dx, dy, dz, dw, dl, dh], dim=1)
    return deltas


def anchor_target(batched_anchors, batched_gt_bboxes, batched_gt_labels, nclasses):
    '''
    ...
    for i in range(batch_size):
        ...
        d1, d2, _, d4 = anchors.size()
        cur_anchors = anchors[:, :, 0, :].reshape(-1, 6)

        assigned_gt_inds = torch.zeros_like(cur_anchors[:, 0], dtype=torch.long)

        gt_centers_x = gt_bboxes[:, 0]  # GT bbox center x coordinates
        gt_centers_y = gt_bboxes[:, 1]  # GT bbox center y coordinates
        
        anchor_centers_x = cur_anchors[:, 0]  # Anchor center x coordinates
        anchor_centers_y = cur_anchors[:, 1]  # Anchor center y coordinates
        
        for gt_idx in range(len(gt_bboxes)):
            dist = (gt_centers_x[gt_idx] - anchor_centers_x) ** 2 + (gt_centers_y[gt_idx] - anchor_centers_y) ** 2
            inside_mask = dist <= 0.25

            assigned_gt_inds[inside_mask] = gt_idx + 1
        ...
        multi_labels = torch.cat([assigned_gt_labels.reshape(d1, d2, 1)], dim=-1).reshape(-1)
        multi_label_weights = torch.cat([assigned_gt_labels_weights.reshape(d1, d2, 1)], dim=-1).reshape(-1)
        multi_bbox_reg = torch.cat([assigned_gt_reg.reshape(d1, d2, 1, -1)], dim=-2).reshape(-1, d4)
        multi_bbox_reg_weights = torch.cat([assigned_gt_reg_weights.reshape(d1, d2, 1)], dim=-1).reshape(-1)
    ...
    rt_dict = dict(
        batched_labels=torch.stack(batched_labels, 0),
        batched_label_weights=torch.stack(batched_label_weights, 0),
        batched_bbox_reg=torch.stack(batched_bbox_reg, 0),
        batched_bbox_reg_weights=torch.stack(batched_bbox_reg_weights, 0),
    )
